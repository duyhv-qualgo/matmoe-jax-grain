...
@nnx.jit(static_argnames=('max_len', 'pad_id', 'eos_id', 'current_mlp_dim', 'current_top_k'))
def generate_fast_greedy_jitted(model, source_ids, src_mask, max_len, pad_id, eos_id, current_mlp_dim=None, current_top_k=None):
    ...
    def loop_body(val):
        ...
        for layer_idx, block in enumerate(model.decoder_blocks):
            ...
            moe_out, _, _ = block.moe(block.ln_3(dec_x), direction_ids, current_mlp_dim=current_mlp_dim, 
                                     current_top_k=current_top_k, deterministic=True)
            dec_x = dec_x + moe_out
...
@nnx.jit(static_argnames=('max_len', 'top_k', 'pad_id', 'eos_id', 'current_mlp_dim', 'current_top_k'))
def generate_fast_sample_jitted(model, source_ids, src_mask, key, max_len, temperature, top_k, top_p, pad_id, eos_id, current_mlp_dim=None, current_top_k=None):
    batch_size = source_ids.shape[0]
    num_layers = model.cfg.num_layers
    num_heads = model.cfg.num_heads
    head_dim = model.cfg.d_model // num_heads
...
@nnx.jit(static_argnames=('max_len', 'pad_id', 'eos_id', 'beam_size', 'current_mlp_dim', 'current_top_k'))
def generate_fast_beam_jitted(model, source_ids, src_mask, max_len, pad_id, eos_id, beam_size=4, current_mlp_dim=None, current_top_k=None):
    length_penalty = 0.6
    batch_size = source_ids.shape[0]
    num_layers = model.cfg.num_layers
    num_heads = model.cfg.num_heads
...
class MoEGenerator:
    ...
    def generate(self, texts, method="greedy", max_len=60, temperature=0.7, top_k=40, top_p=0.9, seed=42,
                 num_beams=4, length_penalty=0.6, current_mlp_dim=None, current_top_k=None, verbose=True):
        ...
        if method == "greedy":
            out_ids, final_i = generate_fast_greedy_jitted(self.model, src_ids, src_mask, max_len, self.pad_id,
                                                           self.eos_id, current_mlp_dim=current_mlp_dim, current_top_k=current_top_k)
        elif method == "beam":
            out_ids, final_i = generate_fast_beam_jitted(self.model, src_ids, src_mask, max_len, self.pad_id,
                                                         self.eos_id, beam_size=num_beams, current_mlp_dim=current_mlp_dim, current_top_k=current_top_k)
        elif method == "sample":
            key = jax.random.key(seed)
            out_ids, final_i = generate_fast_sample_jitted(self.model, src_ids, src_mask, key, max_len, temperature,
                                                           top_k, top_p, self.pad_id, self.eos_id, current_mlp_dim=current_mlp_dim, current_top_k=current_top_k)
        else:
