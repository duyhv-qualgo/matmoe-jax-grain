...
class Expert(nnx.Module):
    ...
    def __call__(self, x, current_mlp_dim=None, deterministic: bool = False):
        ...
        # Scale factor to compensate for reduced capacity
...
class MoELayer(nnx.Module):
    ...
    def __call__(self, x, direction_ids, current_mlp_dim=None, deterministic: bool = False):
        ...
        mask = jax.nn.one_hot(top_k_indices, self.num_experts)
        f_i = jnp.mean(jnp.sum(mask, axis=1), axis=0)
        P_i = jnp.mean(routing_probs, axis=0)
        aux_loss = self.num_experts * jnp.sum(f_i * P_i)
...
class EncoderBlock(nnx.Module):
    ...
    def __call__(self, x, mask, direction_ids, current_mlp_dim=None, sin=None, cos=None, deterministic=False):
        ...
        moe_out, aux_loss, z_loss = self.moe(self.ln_2(x), direction_ids, current_mlp_dim=current_mlp_dim, deterministic=deterministic)
...
class DecoderBlock(nnx.Module):
    ...
    def __call__(self, x, tgt_mask, enc_out, src_mask, direction_ids, current_mlp_dim=None, sin=None, cos=None, deterministic=False):
        ...
        moe_out, aux_loss, z_loss = self.moe(self.ln_3(x), direction_ids, current_mlp_dim=current_mlp_dim, deterministic=deterministic)
        x = x + self.res_dropout(moe_out, deterministic=deterministic)
        return x, aux_loss, z_loss
...
class Encoder(nnx.Module):
    ...
    def __call__(self, x, mask, direction_ids, current_mlp_dim=None, sin=None, cos=None, deterministic=False):
        ...
        for block in self.blocks:
            x, aux_loss, z_loss = block(x, mask, direction_ids, current_mlp_dim=current_mlp_dim, sin=sin, cos=cos, deterministic=deterministic)
...
class Decoder(nnx.Module):
    ...
    def __call__(self, x, tgt_mask, enc_out, src_mask, direction_ids, current_mlp_dim=None, sin=None, cos=None, deterministic=False):
        ...
        total_z_loss = 0.0
        for block in self.blocks:
            x, aux_loss, z_loss = block(x, tgt_mask, enc_out, src_mask, direction_ids, current_mlp_dim=current_mlp_dim, sin=sin, cos=cos,
...
class MoETranslationModel(nnx.Module):
    def __init__(self, cfg: MoEModelConfig, rngs: nnx.Rngs):
        ...
        # 🌟 KEPT: Embedding RMSNorm (Gemma 2 Stabilization)
    ...
    def __call__(self, source_ids, target_ids, src_mask, tgt_mask, current_mlp_dim=None, deterministic=False):
        ...
        enc_out, enc_aux, enc_z = self.encoder(src_emb, src_mask, direction_ids, current_mlp_dim=current_mlp_dim, sin=src_sin, cos=src_cos,
                                               deterministic=deterministic)
        dec_out, dec_aux, dec_z = self.decoder(tgt_emb, tgt_mask, enc_out, src_mask, direction_ids, current_mlp_dim=current_mlp_dim, sin=tgt_sin,
